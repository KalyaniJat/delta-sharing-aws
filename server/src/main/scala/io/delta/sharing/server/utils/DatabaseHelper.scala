...
object DatabaseHelper {
  ...
  def validateUserSubscriptionAndQueryLimitGroup(userId: String, groupName: String): Boolean = {
    ...
    try {
      ...
      // Define SQL Query
      val query = "select subscription_plan,group_name,subscription_pricing_detail,expiration_date,  SUM(queries_used) as totalCount from public.dep_metadata_user_group_subscriptions where group_name = ? group by subscription_plan,group_name,subscription_pricing_detail,expiration_date;"
      // Prepare and execute statement
      preparedStatement = connection.prepareStatement(query)
  ...
  def updateUserQueryAuditTable(userId: String, productCatalogId: String, productCatalogName: String, groupName: String, subscriptionPlan: String, groupId:String=null): Unit = {
    ...
    try {
      ...
      if (resultSet.next()) {
        ...
        val insertQuery = "INSERT INTO public.user_query_audit (user_id, catalog_id, catalog_name, query_count, time_created,group_name, subscription_plan) VALUES (?, ?, ?, ?, ?,?,?)"
